		...
		if no.Parent == no.Parent.Parent.Left {
			...
			if y.color == RED {
				...
				no.Parent.Parent.color = RED
				 NO = no.parent.parent // looped up from the self-balance.
			} else {
				if no == no.Parent.Right {
					//
					 // Situation 5: Background
					 / / Direct left, then do 3 (discoloration -> right)
		 ...
		 } else {// For the right child of the father's node, like the left child, change the steering.
